...
private:
    ...
    Operand DecodeOperandCI(uint16_t insn, bool immSigned) const
    {
        ...
        {
            immSigned ?
                SignExtend<int64_t>(6,
                    Pick(insn, 12, 1) << 5 |
                    Pick(insn, 2, 5)
                ) :
                ZeroExtend<int64_t>(6,
                    Pick(insn, 12, 1) << 5 |
                    Pick(insn, 2, 5)
                ), // imm
            Pick(insn, 7, 5), // rd
